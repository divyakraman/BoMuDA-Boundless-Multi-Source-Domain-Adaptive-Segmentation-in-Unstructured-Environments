...
from model.openset_model import *
...
SNAPSHOT_DIR = './snapshots/idd_openset/'
WEIGHT_DECAY = 0.0005
#THRESHOLD = 0.9 
...
def get_arguments():
    ...
    parser.add_argument("--pseudoWeight", type=str, default=pseudoWeight,
                        help="pseudo weight")
    parser.add_argument("--KLWeight", type=str, default=KLWeight,
                        help="KL weight")
...
def main():
    ...
    closedset_finalmodel = torch.load('snapshots/idd_multi3source_combinedbddbase_iteration3/IDD_30000.pth')
    ...
    for i_iter in range(0, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            pred_closed_final = closedset_finalmodel(pred_3source)

            #Pseudo labels generation
            pred = pred_closed_final.detach()
            pred = pred.cpu()
            pred = pred.numpy()
            pred = pred[0,:,:,:]
            pred = np.exp(pred)/np.sum(np.exp(pred),0)
            pred_prob = np.max(pred,0)
            pred = np.argmax(pred,0)
            #'''
            #Car to auto
            b = pred_prob<t11 #Auto
            c = pred==13 #Auto
            b = b.astype(int)
            c = c.astype(int)
            pred[b+c==2]=20 #Auto
            ...
            #Person to animal
            a = pred_prob<t21 #Animal
            d = pred==11 #Animal
            a = a.astype(int)
            d = d.astype(int)
            pred[a+d==2]=19 #Animal

            #Rider to animal
            a = pred_prob<t22 #Animal
            d = pred==12 #Animal
            a = a.astype(int)
            d = d.astype(int)
            pred[a+d==2]=19 #Animal  
            ...
            pseudo_labels = pred   
            pseudo_labels = np.expand_dims(pseudo_labels, 0)        
            
            pred_final = model(pred_closed_final)

            loss_seg1 = args.pseudoWeight * loss_calc(pred_final, pseudo_labels, args.gpu)
            loss = loss_seg1

            loss = loss + args.KLWeight* (0.7 * kl_loss(pred_final[:,20,:,:],pred_closed_final[:,13,:,:]) + 0.15 * kl_loss(pred_final[:,20,:,:],pred_closed_final[:,14,:,:]) + 0.15 * kl_loss(pred_final[:,20,:,:],pred_closed_final[:,15,:,:]))
            #Car, truck, bus for autorickshaw
            loss = loss + args.KLWeight* (0.65 * kl_loss(pred_final[:,19,:,:],pred_closed_final[:,11,:,:]) + 0.1 * kl_loss(pred_final[:,19,:,:],pred_closed_final[:,13,:,:]) + 0.25 * kl_loss(pred_final[:,19,:,:],pred_closed_final[:,12,:,:]))
            #Person, rider car for animal            
